...
def solve_directions(start_coordinates, end_coordinates, map):
    ...
    for i in range(num_coordinates):
        ...
        if start_coordinate[1] ==  end_coordinate[1]:       # horizontal
            ...
            x_step = 1 if start_coordinate[0] < end_coordinate[0] else -1
            y_step = 1 if start_coordinate[1] <  end_coordinate[1] else -1
            x_counter = start_coordinate[0]
            y_counter = start_coordinate[1]
            while x_counter != end_coordinate[0] + x_step and y_counter != end_coordinate[1] + y_step:
                map[x_counter][y_counter] += 1
                x_counter += x_step
                y_counter += y_step
